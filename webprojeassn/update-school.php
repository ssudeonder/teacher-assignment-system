...
$id = $_GET['id'];
$selectSql = "SELECT * FROM school where id=".$id;
$selectResult = $conn->query($selectSql);
if ($selectResult->num_rows > 0) {
    $school = $selectResult->fetch_assoc();
} else {
    header("Location:schools.php");
}
if (count($_POST) > 0) {
    ...
    if ($name && $music >= 0 && $history >=0 && $computer >= 0 && $math >=0 && $physics >=0) {
        $sql = "UPDATE school SET name='".$name."', physics=".$physics.", math=".$math.", computer=".$computer.", history=".$history.", music=".$music." where id=".$school['id'].";";
...
<form name="form" method="post" action="" align="center">
  <div>
    ...
    <div><input type="text" name="name" value="<?= $school['name'] ?>"></div>
  </div>
  <div>
    <div><label>Physics Section Count:</label></div>
    <div><input type="number" name="physics" value="<?= $school['physics'] ?>"></div>
  </div>
  <div>
    <div><label>Math Section Count:</label></div>
    <div><input type="number" name="math" value="<?= $school['math'] ?>"></div>
  </div>
  <div>
    <div><label>Computer Section Count:</label></div>
    <div><input type="number" name="computer" value="<?= $school['computer'] ?>"></div>
  </div>
  <div>
    <div><label>History Section Count:</label></div>
    <div><input type="number" name="history" value="<?= $school['history'] ?>"></div>
  </div>
  <div>
    <div><label>Music Section Count:</label></div>
    <div><input type="number" name="music" value="<?= $school['music'] ?>"></div>
  </div>
